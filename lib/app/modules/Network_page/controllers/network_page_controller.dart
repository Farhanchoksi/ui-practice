...
class NetworkPageController extends GetxController {
  ...
  final RxList<Map<String, dynamic>> profileUsers = <Map<String, dynamic>>[].obs;
  final RxString selectedFilter = 'Profile'.obs; // 'All', 'Profile', 'Session'
  ...
  void applyFilter(String filter) {
    ...
    if (filter == 'All') {
      userList.value = List.from(profileUsers);
    } else if (filter == 'Profile') {
      userList.value = profileUsers
          .where((user) => user['type'].toString().toLowerCase() == 'profile')
          .toList();
    } else if (filter == 'Session') {
      userList.value = profileUsers
          .where((user) => user['type'].toString().toLowerCase() == 'session')
          .toList();
    } else {
      userList.value = profileUsers
          .where((user) => user['type'].toString().toLowerCase() == filter.toLowerCase())
          .toList();
    }
  ...
  void searchUsers(String query) {
    ...
    Future.delayed(Duration(milliseconds: 300), () {
      ...
      if (selectedFilter.value == 'Profile') {
        baseList = List.from(profileUsers);
      } else {
        baseList =
            profileUsers
                .where(
